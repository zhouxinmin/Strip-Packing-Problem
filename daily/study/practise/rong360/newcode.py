...
#
# browse_history
browse_history_train = pd.read_csv('H:/Datamining/rong360/credit/train/browse_history_train.txt',
                                       header = None)
browse_history_test = pd.read_csv('H:/Datamining/rong360/credit/test/browse_history_test.txt',
                                       header = None)
...
#
# bill_detail
bill_detail_train = pd.read_csv('H:/Datamining/rong360/credit/train/bill_detail_train.txt',
                                       header = None)
bill_detail_test = pd.read_csv('H:/Datamining/rong360/credit/test/bill_detail_test.txt',
                                       header = None)
...
# loan_time
loan_time_train = pd.read_csv('H:/Datamining/rong360/credit/train/loan_time_train.txt',
                              header = None)
loan_time_test = pd.read_csv('H:/Datamining/rong360/credit/test/loan_time_test.txt',
                              header = None)
...
target = pd.read_csv('H:/Datamining/rong360/credit/train/overdue_train.txt',
                         header = None)
...
# 这里用神经网络作预测
clf = MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(20, 13, 2), random_state=1)
...
# print classification_report(test_y, pred_test)
# # 输出测试集用户逾期还款概率，predict_proba会输出两个概率，取‘1’的概率
# pred = svm.predict_proba(test)

# 这里用随机森林预测
random = RandomForestClassifier(random_state=123)
random.fit(train_X, train_y)
# 给出交叉验证集的预测结果，评估准确率、召回率、F1
pred_test = random.predict(test_X)
print classification_report(test_y, pred_test)
# 输出测试集用户逾期还款概率，predict_proba会输出两个概率，取‘1’的概率
pred = random.predict_proba(test)
