...
SizeMin = size(Minority_features,1);                % 得到minority的行数
min_clusters = (1:SizeMin)';                        % 得到一列minority行的矩阵

%% Clustering the majority class using Hierachical Clustering
maj_clusters = Orig_agg_cluster(Majority_features, CThresh);        % 对majority聚类

% Kmaj = size(unique(maj_clusters),1);
% m_each_maj = histc(maj_clusters,1:Kmaj);

Whole_data_min = [Minority_features; Majority_features];            % 合并矩阵
D = pdist(Whole_data_min,'euclidean');                              % 计算行向量之间的距离
point_dist_min = squareform(D);                                     % 得到各行向量距离的方阵
...
function labels  = inside_AggCluster(data, same_clusters, other_clusters, point_dist_whole, CThresh)    % 全部数据矩阵‘， minority行1列矩阵， majority矩阵，行向量距离矩阵， 控制参数
Num_Reject = 0; 
N = size(data,2);                       % 数据矩阵的行数
Exist_Clus = unique(same_clusters);     
M = size(Exist_Clus ,1);                % minority的行数 == M

% the distance between each pair of points
point_dist = point_dist_whole(1:N,1:N);                 % 取方阵
point_dist2 = point_dist;                               % 相等
for i=1:N
    point_dist2(i,i) = 100;                             % 令A-A点的距离由0改为100
end

% Measuring the threshold
thresh = mean(median(point_dist2)).* CThresh;           % 得到thresh值
